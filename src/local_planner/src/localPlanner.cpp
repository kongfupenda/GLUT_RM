  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) { //进入的总判断，有点云才能进入
      ...
      if (autonomyMode) {     //autonomyMode 自主模式 true
        ...
        relativeGoalDis = sqrt(relativeGoalX * relativeGoalX + relativeGoalY * relativeGoalY);    //小车与目标点的距离
        joyDir = atan2(relativeGoalY, relativeGoalX) * 180 / PI;    //反正切函数（atan2）算斜边角 单位：角度
        // cout << "joyDir = " << joyDir << endl;
        if (!twoWayDrive) {           //twoWayDrive is ture 不执行这个
                ...
                for (int j = 0; j < blockedPathByVoxelNum; j++) {
                  if (h > obstacleHeightThre || !useTerrainAnalysis) {
                    ...
                    if (pathPenaltyList[pathNum * rotDir + correspondences[ind][j]] < h && h > groundHeightThre) {
                      pathPenaltyList[pathNum * rotDir + correspondences[ind][j]] = h;      //当相对高度intensity小于一定障碍物高度阈值obstacleHeightThre 且大于地面高度groundHeightThre的情况下。相当于此处存在坡度则 pathPenaltyList 惩罚值将设置为该相对高度
                    }
                  }
                }
              }
...
//------------------------------------------------对36个方向的paths进行评分（筛除了障碍物的路径的）--------------------------------------------------------------------
        ...
        for (int i = 0; i < 36 * pathNum; i++) {    //pathNum = 343   343条路径，36个方向
          ...
          if (clearPathList[i] < pointPerPathThre) {      //pointPerPathThre = 2  
            ...
            float score = (1 - sqrt(sqrt(dirWeight * dirDiff))) * penaltyScore;   //无约束版本    修改评分规则，以最短路径作为评分标准

            // cout <<"score = "<< score << " 1 - sqrt(sqrt(dirWeight * dirDiff)) = " << (1 - sqrt(sqrt(dirWeight * dirDiff))) << " rotDirW = "<< rotDirW << endl;
...
//-----------------------------------------------------选择得分最高的路径----------对36个方向的startpaths进行选取---------------------------------------
        float maxScore = 0;
...
//---------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if (selectedGroupID >= 0) {
          ...
          float rotAng = (10.0 * rotDir - 180.0) * PI / 180;      //rotAng-3.14159和0两个值，当路径在前面就是0，后面就是-3.14159
          // joyDir = joyDir * PI / 180;      //直接用joyDir没有避障了，突然发现他本身就好像没有避障
          ...
          for (int i = 0; i < selectedPathLength; i++) {
            ...
            if (dis <= pathRange / pathScale && dis <= relativeGoalDis / pathScale) {
              path.poses[i].pose.position.x = pathScale * (cos(rotAng) * x - sin(rotAng) * y);    //joyDir 方案①：合适的旋转,利用3来移动   他是先选择路径，然后才根据角度旋转路径的
              path.poses[i].pose.position.y = pathScale * (sin(rotAng) * x + cos(rotAng) * y);    //方案②：直接做一个无约束离散点路径
