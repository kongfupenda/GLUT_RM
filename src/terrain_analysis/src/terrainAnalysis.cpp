...
void odometryHandler(const nav_msgs::Odometry::ConstPtr &odom) {
  ...
  if (noDataInited == 1) {
    float dis = sqrt((vehicleX - vehicleXRec) * (vehicleX - vehicleXRec) +    //以第一帧的位置为原点建立坐标系，算第二帧小车的距离
                     (vehicleY - vehicleYRec) * (vehicleY - vehicleYRec));
    if (dis >= noDecayDis)      //两帧位置差太大就考虑
      noDataInited = 2;
  }
}
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloud2) {
  ...
  laserCloud->clear();  
  pcl::fromROSMsg(*laserCloud2, *laserCloud); //fromROSMsg函数将laserCloud2中的ROS消息转换为PCL点云，并将结果存储在laserCloud中

  pcl::PointXYZI point;
  laserCloudCrop->clear();
  int laserCloudSize = laserCloud->points.size();
  for (int i = 0; i < laserCloudSize; i++) {  //点云遍历
    ...
    float dis = sqrt((pointX - vehicleX) * (pointX - vehicleX) +    //以小车目前点位置建立坐标系，小车位置为(0,0,0),算点云的距离
                     (pointY - vehicleY) * (pointY - vehicleY));

    if (pointZ - vehicleZ > minRelZ - disRatioZ * dis &&  //minRelZ - disRatioZ * dis基于Z坐标的动态阈值计算,距离越远处理点云的高度越小
        pointZ - vehicleZ < maxRelZ + disRatioZ * dis &&  //距离越远处理点云的高度越越高            点云越远，处理的扇形范围越大
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subOdometry =
      nh.subscribe<nav_msgs::Odometry>("/state_estimation", 5, odometryHandler);  //输入

  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>(         //输入
      "/registered_scan", 5, laserCloudHandler);

  ros::Subscriber subJoystick =
      nh.subscribe<sensor_msgs::Joy>("/joy", 5, joystickHandler);           //遥控器

  ros::Subscriber subClearing =
      nh.subscribe<std_msgs::Float32>("/map_clearing", 5, clearingHandler);   //

  ros::Publisher pubLaserCloud =
      nh.advertise<sensor_msgs::PointCloud2>("/terrain_map", 2);    //输出
